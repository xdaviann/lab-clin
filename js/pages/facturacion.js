...
const FacturacionPage = (() => {
      ...
      <main class="content" id="page-content">
        <div class="page-header">
          ...
          <div class="page-actions">
            <button class="btn btn-secondary btn-sm" onclick="FacturacionPage.refresh()">
              ${Icons.refreshCw()} Actualizar Datos
            </button>
            <button class="btn btn-secondary btn-sm" onclick="FacturacionPage.openTasaModal()">
              ${Icons.dollarSign()} Actualizar Tasa
            </button>
          </div>
        </div>
  ...
  function processPayment(invoiceId) {
    ...
    const referencia = (document.getElementById('pay-reference')?.value || '').trim();
  ...
  function openTasaModal() {
    ...
    const lastUpdate = CurrencyService.getLastUpdate();
    const fechaUpdate = lastUpdate ? new Date(lastUpdate).toLocaleString('es-ES') : 'Sin registro';
    ...
    Modal.open({
      ...
      content: `
        <div style="margin-bottom: var(--spacing-lg); padding: var(--spacing-base); background: var(--color-surface-50); border-radius: var(--radius-lg);">
          <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
            <span style="color: var(--color-surface-500); font-size: var(--font-size-sm);">Tasa actual</span>
            <span style="font-weight: var(--font-weight-bold); color: var(--color-primary-600);">Bs. ${tasaActual.toFixed(2)}</span>
          </div>
          <div style="display: flex; justify-content: space-between;">
            <span style="color: var(--color-surface-500); font-size: var(--font-size-sm);">Última actualización</span>
            <span style="font-size: var(--font-size-sm); color: var(--color-surface-400);">${fechaUpdate}</span>
          </div>
        </div>
        <div class="form-group" style="margin-bottom: var(--spacing-base);">
          <label class="form-label">Nueva tasa BCV (Bs. por 1 USD) <span class="required">*</span></label>
          <input type="number" id="nueva-tasa-input" step="0.01" min="0" placeholder="Ej: 65.50" value="" style="font-size: var(--font-size-lg); font-weight: var(--font-weight-bold); text-align: center;" />
        </div>
      ...
      footer: `
        <button class="btn btn-secondary" onclick="Modal.close()">Cancelar</button>
        <button class="btn btn-primary" onclick="FacturacionPage.saveTasa()">
          ${Icons.check()} Guardar Tasa
        </button>
      `
    });
  }
  ...
  function saveTasa() {
    ...
    if (CurrencyService.setTasaManual(nuevaTasa)) {
      Modal.close();
      Toast.success(`Tasa BCV actualizada a Bs. ${nuevaTasa.toFixed(2)}`);
      /* Marcar tasa como actualizada hoy para notificaciones */
