...
const CurrencyService = (() => {
  ...
  async function fetchTasaBCV(silent = true) {
    try {
      ...
      if (data && data.monitors && data.monitors.usd) {
        ...
        if (!silent) Toast.success(`Tasa BCV actualizada: Bs. ${tasaBCV.toFixed(2)}`);
        return tasaBCV;
      }
    } catch (error) {
      /* Si la API falla, usar la tasa manual existente */
      console.warn('No se pudo obtener tasa BCV, usando tasa manual:', error.message);
      if (!silent) Toast.warning(`No se pudo obtener tasa BCV automática. Tasa actual: Bs. ${tasaBCV.toFixed(2)}`);
    }
    return tasaBCV;
  }
  ...
  function renderCurrencyWidget() {
    return `
      <div style="display: flex; align-items: center; gap: var(--spacing-sm); font-size: var(--font-size-xs);">
        <select id="currency-selector" onchange="CurrencyService.setMonedaActiva(this.value); window.dispatchEvent(new Event('hashchange'));" style="width: auto; min-width: 80px; font-size: var(--font-size-xs); padding: 4px 8px;">
