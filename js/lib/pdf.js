...
const PdfService = (() => {
  ...
  function addHeader(doc, title, subtitle) {
    ...
    // Add Logo & Name
    const logoEl = document.querySelector('.sidebar-logo img') || document.querySelector('.login-logo');
    if (logoEl) {
      try {
        doc.addImage(logoEl, 'PNG', 14, 6, 22, 22);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.setTextColor(255, 255, 255);
        doc.text('Laboratorio Clínico Ordoñez VE, C.A.', 40, 16);
        doc.setFontSize(8);
        doc.setTextColor(160, 160, 180);
        doc.text('SISTEMA DE GESTIÓN DE LABORATORIO CLÍNICO', 40, 23);
      } catch(e) {
        // Fallback si la imagen no se puede cargar
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(14);
        doc.setTextColor(255, 255, 255);
        doc.text('Laboratorio Clínico Ordoñez VE, C.A.', 14, 16);
        doc.setFontSize(8);
        doc.setTextColor(160, 160, 180);
  ...
  function addFooter(doc) {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Laboratorio Clínico Ordoñez VE, C.A. — Documento generado el ${new Date().toLocaleString('es-ES')} — Página ${i} de ${pageCount}`,
  ...
  function generatePaymentReceiptPdf(invoice, payment, shouldPrint = false) {
    ...
    if (logoEl) {
      try {
        doc.addImage(logoEl, 'PNG', 10, 4, 16, 16);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        doc.setTextColor(255, 255, 255);
        doc.text('Lab. Clínico Ordoñez VE', 28, 12);
        doc.setFontSize(7);
        doc.setTextColor(180, 180, 200);
        doc.text('Comprobante de Pago', 28, 18);
      } catch(e) {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(12);
        doc.setTextColor(255, 255, 255);
        doc.text('Lab. Clínico Ordoñez VE', 10, 14);
        doc.setFontSize(8);
        doc.setTextColor(180, 180, 200);
  ...
  /* ── Generar Orden PDF ── */
  function generateOrderPdf(order, pruebas, shouldPrint = false) {
    const JsPDF = getJsPDF();
    if (!JsPDF) return;

    const doc = new JsPDF();
    const tasa = CurrencyService.getTasa();
    let y = addHeader(doc, `ORDEN DE LABORATORIO ${order.id}`, `Paciente: ${order.paciente}`);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    doc.text(`Fecha: ${order.fecha}`, 14, y); y += 5;
    doc.text(`Prioridad: ${order.prioridad}`, 14, y); y += 5;
    doc.text(`Estado: ${order.estado}`, 14, y); y += 10;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.text(`Total (USD): $${order.total.toFixed(2)}`, 130, y); y += 6;
    doc.text(`Total (Bs.): Bs. ${(order.total * tasa).toFixed(2)}`, 130, y); y += 10;

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text(`Tasa BCV referencial: Bs. ${tasa.toFixed(2)}`, 14, y);

    addFooter(doc);
    handleOutput(doc, `Orden_${order.id}`, shouldPrint);
    Toast.success(`PDF de la Orden ${order.id} descargado`);
  }
